...
local function hasattr(mod, attr)
    return mod[attr]
end
...
do
    ...
    function proc_read(proc, stderr, reader_type, len)
        ...
        if rc == FFI_BAD_CONTEXT then
            error(ffi_str(errbuf, errbuf_size[0]))
        end
...
local function proc_kill(proc, signal)
    ...
    if type(signal) ~= "number" then
        error("bad signal value: " .. tostring(signal), 2)
    end
...
do
    ...
    function pipe_spawn(args, opts)
        ...
        if args_type == "table" then
            ...
            if nargs == 0 then
                error("bad args arg: nonempty table expected", 2)
            end
