...
bad args arg: nonempty table expected
...
--- response_body
bad shutdown argument: read
bad shutdown argument: 0
...
--- config
    location = /t {
        content_by_lua_block {
            ...
            ngx.say("shutdown stderr after read")
            local proc = ngx_pipe.spawn(args, opts)
            proc:stderr_read_line()
            ...
            ngx.say("shutdown stderr before read")
            local proc = ngx_pipe.spawn(args, opts)
            shutdown(proc, 'stderr')
            read_proc(proc)
        }
...
stdout: closed
stderr: closed
shutdown stderr after read
stdout: closed
stderr: closed
shutdown stdout before read
stdout: closed
stderr: closed
shutdown stderr before read
stdout: closed
stderr: closed
...
1
--- error_log
lua pipe execvp() failed while executing /exit (2: No such file or directory)
...
--- response_body_like
lua pipe execvp\(\) failed while executing no-such-cmd \(2: No such file or directory\)
